...
def prepare_vindrmammo_birad(df: pd.DataFrame, split_name: str) -> pd.DataFrame:
    """Prepare the VinDr-Mammo dataset for multi-view BI-RADS classification.

    Groups images by study_id, keeping only studies with all 4 views.
    """
    ...
    df_split_agg = filter_complete_studies(df_split_agg)
    logger.info(f"Multi-view processing: {len(df_split_agg)} complete studies in {split_name} set")
    return df_split_agg


def create_multi_view_structure(df_agg: pd.DataFrame, output_dir: str, split: str,
                               src_images_folder: str, use_symlink: bool = False) -> None:
...
def main():
    ...
    df_train_agg = prepare_vindrmammo_birad(df_train_raw, "training")
    df_test_agg = prepare_vindrmammo_birad(df_test_raw, "test")
