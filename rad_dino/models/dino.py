...
class DinoClassifier(nn.Module):
    def __init__(self, 
        ...
        Args:
            backbone: Pre-trained DINO backbone model (e.g., ViT)
    ...
    def _single_view_input_reshape(self, x: torch.Tensor) -> tuple[torch.Tensor, int]:
        """
        Single view reshaping strategy.
    ...
    def _multi_view_input_reshape(self, x: torch.Tensor) -> tuple[torch.Tensor, int]:
        ...
        x_reshaped = x.reshape(batch_size * num_views, x.shape[2], x.shape[3], x.shape[4])
        return x_reshaped, num_views
    ...
    def _single_view_fusion(self, cls_tokens: torch.Tensor, batch_size: int, num_views: int) -> torch.Tensor:
        """
        Single view fusion - return the token directly.
        
        Args:
            cls_tokens: CLS tokens from backbone [B, D]
            batch_size: Batch size
            num_views: Number of views (always 1 for single view)
            
        Returns:
            Original CLS tokens [B, D]
        """
        return cls_tokens
    
    def _mean_fusion(self, cls_tokens: torch.Tensor, batch_size: int, num_views: int) -> torch.Tensor:
        """
        Mean fusion across views - simple average.
        
        Args:
            cls_tokens: CLS tokens from backbone [B*V, D]
            batch_size: Batch size
        ...
        """
        # Reshape: [B*V, D] -> [B, V, D]
        cls_tokens = cls_tokens.view(batch_size, num_views, self.embed_dim)
        return cls_tokens.mean(dim=1)
    
    def _weighted_mean_fusion(self, cls_tokens: torch.Tensor, batch_size: int, num_views: int) -> torch.Tensor:
        """
        ...
        return (cls_tokens * w[None,:,None]).sum(dim=1)
    
    def _mlp_adapter_fusion(self, cls_tokens: torch.Tensor, batch_size: int, num_views: int) -> torch.Tensor:
        """
        ...
        for i in range(num_views):
            adapted_cls_tokens.append(self.view_adapters[i](cls_tokens[:,i,:]))
        adapted_cls_tokens = torch.stack(adapted_cls_tokens, dim=1)  # [B, V, D]
        
        # Apply LayerNorm to normalized adapted features
        adapted_cls_tokens = self.layer_norm(adapted_cls_tokens)
        
        # Concatenate and apply view fusion MLP layer
        return self.view_fusion_layer(adapted_cls_tokens.view(batch_size, num_views * self.embed_dim))

    def forward(self, x):
        """
        Forward pass through the DINO classifier.
        
        Supports both single-view and multi-view inputs:
        - Single-view: x has shape [B, C, H, W]
        - Multi-view: x has shape [B, V, C, H, W] where V is number of views
        
        Args:
            x: Input tensor, either single-view or multi-view
        ...
        # Extract CLS tokens from the last hidden state
        ...
        fused_features = fusion_strategy(cls_tokens, batch_size, getattr(self, 'num_views', 1))
...
if __name__ == "__main__":
    ...
    # Load pre-trained DINO backbone
    backbone = AutoModel.from_pretrained('facebook/dinov2-base')
    # Alternative: backbone = AutoModel.from_pretrained("microsoft/rad-dino")
    ...
    # Configure fine-tuning: unfreeze last 2 layers
